import { HoodPropsList, HoodId, HoodIds, HoodPropsMap, HoodProps, HoodUIPropsMap, HoodUIProps } from './types';
...
export const getHoodUIProps = (state: State): HoodUIPropsMap => state[hoodUIProps] || {};
export const getHoodUIPropsByIds = (state: State, { ids = [] }: { ids: HoodIds }): HoodPropsMap => {
  const hoodProps: HoodUIPropsMap = getHoodUIProps(state);
  return Object
    .values(hoodProps)
    .reduce((acc, hood: HoodUIProps) => {
      if (ids.indexOf(hood.id) === -1) {
        return acc;
      }
...
export const getHoodUIPropsById = (state: State, { id = '' }: { id: HoodId }) => {
  const propsList = getHoodUIPropsByIds(state, { ids: [id] });
  return propsList[id];
};
export const getHoodUIPropsAsIds = (state: State) => Object.keys(getHoodUIProps(state));
